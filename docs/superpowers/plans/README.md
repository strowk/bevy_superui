...
Copy the block below as the opening prompt for the next session (it targets **Plan 4**,
the first unstarted row):

> Read `docs/superpowers/specs/2026-07-18-bevy-superui-design.md` and
> `docs/superpowers/plans/README.md`. Plans 1 (`superui_dom`), 2 (`superui_html`), and 3
> (`superui_js` + `superui_api`) are done and merged to `main` (see their plan docs for
> the delivered public surface). Write and execute Plan 4 of the 6-plan series:
> `superui_css` (fork of `bevy_flair` 0.6 for Bevy 0.17, extended for real HTML
> element/attribute selectors and `:hover`/`:focus`/`:checked`).
