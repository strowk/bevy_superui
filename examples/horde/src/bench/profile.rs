...
//!     --profile --preset stress --enemy-cap 400 --frames 500 --warmup 300
//! ```
...
/// rendered, instead of dying and dropping to the cheap `GameOver` screen. Without
/// this, a long warmup under a 400-enemy stress swarm ends in GameOver and the
/// profile measures an almost-empty UI. Runs in `First`, before any sim damage.
...
pub fn run_profile(sim: SimConfig, frames: usize, warmup: usize) {
    ...
    // God-mode the player so the swarm stays on-screen in `Playing` (see above).
    app.add_systems(First, keep_player_alive);

    // Warm up with recording off (mount, swarm build-up, steady state).
    for _ in 0..warmup {
        app.update();
    }
    ...
    if a.per_system.is_empty() {
        println!(
            "profile: no system spans were recorded.\n\
             ...
             \n    cargo run --release -p horde --features bench,bevy/trace --bin horde-bench -- --profile ...\n"
        );
        return;
    }
